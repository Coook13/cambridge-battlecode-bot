...
class Player:
    ...
    def _bootstrap_enemy_core_guess(self, c):
        ...
        if c.get_map_width() == 50 and c.get_map_height() == 30:
            if self.core_pos.x == 3 and self.core_pos.y == 16:
                self.enemy_symmetry = 'MAP_LARGE2_VERTICAL'
                self.enemy_core_guess = Position(46, 16)
                return
            if self.core_pos.x == 46 and self.core_pos.y == 16:
                self.enemy_symmetry = 'MAP_LARGE2_VERTICAL'
                self.enemy_core_guess = Position(3, 16)
                return
