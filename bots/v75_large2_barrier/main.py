...
class Player:
    ...
    def _builder(self, c):
        ...
        if self._try_large2_route_barrier(c, pos):
            return
    ...
    def _try_large2_route_barrier(self, c, pos):
        if self.core_pos is None:
            return False
        if not (
            c.get_map_width() == 50
            ...
            return False
        if c.get_current_round() < 18 or c.get_current_round() > 120:
            return False
        if c.get_action_cooldown() > 0:
            return False
        ...
        for x, y in (
            ...
            target = Position(x, y)
            try:
                if pos.distance_squared(target) <= 2 and c.can_build_barrier(target):
