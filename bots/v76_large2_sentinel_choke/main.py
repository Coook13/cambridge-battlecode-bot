...
class Player:
    ...
    def _builder(self, c):
        ...
        if self._try_large2_choke_sentinel(c, pos):
            return
    ...
    def _try_large2_choke_sentinel(self, c, pos):
        if self.core_pos is None:
            return False
        if not (
            c.get_map_width() == 50
            ...
            return False
        if c.get_current_round() < 35 or c.get_current_round() > 150:
            return False
        if c.get_action_cooldown() > 0:
            return False

        for x, y, facing in (
            (9, 19, Direction.SOUTHEAST),
            (5, 13, Direction.SOUTHEAST),
        ):
            target = Position(x, y)
            try:
                if pos.distance_squared(target) <= 2 and c.can_build_sentinel(target, facing):
                    c.build_sentinel(target, facing)
                    return True
            except Exception:
                pass
            try:
                if pos.distance_squared(target) <= 2 and c.can_build_gunner(target, facing):
