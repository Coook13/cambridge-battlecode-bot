...
# ---- economy phase ----
# v62_fastwave: smaller faster waves
WAVE_SIZE = 2                # was 4
WAVE_INTERVAL = 75           # was 150
SCOUT_WAVE_SLOT = 0
...
class Player:
        ...
        for d in ALL_DIRS:
            ...
            try:
                if c.can_spawn(sp):
                    dx = sp.x - ex
                    dy = sp.y - ey
                    candidates.append((dx * dx + dy * dy, d, sp))
            except Exception:
                pass
        candidates.sort(key=lambda t: t[0])

        for _d2, _d, sp in candidates:
            try:
                c.spawn_builder(sp)
    ...
    def _builder(self, c):
        ...
        if self._try_large2_core_launcher(c, pos):
            return
    ...
    def _try_large2_core_launcher(self, c, pos):
        if self.core_pos is None:
            return False
        if not (
            c.get_map_width() == 50
            and c.get_map_height() == 30
            and self.core_pos.x == 3
            and self.core_pos.y == 16
        ):
            return False
        if c.get_current_round() > 70 or c.get_action_cooldown() > 0:
            return False

        # Build a launcher just east of the core and let it throw fresh
        # attackers past the first large2 choke. This mirrors the top-replay
        # launcher idea without paying for the old full defender chain.
        launcher_pos = Position(5, 16)
        try:
            if c.can_build_launcher(launcher_pos):
                c.build_launcher(launcher_pos)
                return True
        except Exception:
            pass
        return False
    ...
    def _launcher_throw(self, c):
        ...
        best = None
        best_score = None
        for bp in c.get_nearby_tiles():
            try:
                if pos.distance_squared(bp) > 2:
                    continue
                if c.get_tile_builder_bot_id(bp) is None:
                    continue
            except Exception:
                continue

            for tp in c.get_attackable_tiles():
                try:
                    if not c.can_launch(bp, tp):
                        continue
                except Exception:
                    continue
                # default_large2 A only: throw east, avoiding the wall teeth.
                score = (-tp.x, abs(tp.y - 16), tp.distance_squared(Position(46, 16)))
