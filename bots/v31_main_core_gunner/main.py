...
class Player:
    def __init__(self):
        ...
        self.map_w = None
        self.map_h = None
        self._small_core_gunner_done = False
    ...
    def _has_near_core_gunner(self, c: Controller) -> bool:
        if self.core_pos is None:
            return False
        my_team = c.get_team()
    ...
    def _try_small_map_core_gunner(self, c: Controller) -> bool:
        if self._small_core_gunner_done or self.role != ROLE_ECONOMY:
            return False
        if self.core_pos is None or c.get_action_cooldown() != 0:
            return False
        if not isinstance(self.map_w, int) or not isinstance(self.map_h, int):
            return False
        if self.map_w * self.map_h > 400:
            self._small_core_gunner_done = True
            return False
        ...
        try:
            ...
            cost_ti, _ = c.get_gunner_cost()
        except GameError:
            return False
        if ti < cost_ti + 300:
            return False

        if self._has_near_core_gunner(c):
            self._small_core_gunner_done = True
            return False
        ...
        try:
            ...
            return False
        if cur.distance_squared(self.core_pos) > ACTION_RADIUS_SQ:
            return False

        cx, cy = self.core_pos.x, self.core_pos.y
        enemy = Position(self.map_w - 1 - cx, self.map_h - 1 - cy)
        candidates = []
        for dx in (-2, -1, 0, 1, 2):
            for dy in (-2, -1, 0, 1, 2):
                ...
                if cur.distance_squared(target) > ACTION_RADIUS_SQ:
                    continue
                direction = target.direction_to(enemy)
                candidates.append((abs(dx) + abs(dy), target, direction))
        candidates.sort(key=lambda item: item[0])

        for _dist, target, direction in candidates:
            try:
                if c.can_build_gunner(target, direction):
    ...
    def _run_bot(self, c: Controller):
        ...
        if self.role == ROLE_DEFENDER:
            ...
            if not isinstance(self.builder_state, EconomyState):
                self.builder_state = EconomyState(self.core_pos)
            if self._try_small_map_core_gunner(c):
                return
