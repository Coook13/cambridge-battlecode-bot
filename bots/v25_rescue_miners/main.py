...
class Player:
    ...
    def _run_core(self, c: Controller):
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            # v25_rescue_miners: asymmetric-map escape hatch. If the economy
            # has not found enough harvesters to trip defense_complete, send a
            # couple of simple miners anyway. This targets installed maps like
            # dna where Team A otherwise mines zero and never unlocks combat.
            if (
                not self.defense_complete
                and rnd >= 90
                and miner_count < self.target_miner_count_early
                and harvester_count < 2
                and ti >= builder_cost + 80
            ):
