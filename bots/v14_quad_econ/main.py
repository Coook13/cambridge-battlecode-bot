...
def _max_raiders_for_round(rnd: int) -> int:
    ...
    v13_swarm_attack: raised brackets to match #1 team's volume.
    They had 22 builders over 2000 rounds with bursts of 4 every
    ~80-100 rounds. We mirror that here.
    """
    if rnd < 200:
        return 2
    if rnd < 400:
...
class Player:
    def __init__(self):
        ...
        # v14_quad_econ: target_economy_count is map-scaled in _init_game.
        # #1 team replay had 4 econ-style builders spawned at r=0-3 on
        # a 50x41 map. We mirror that for large maps but keep small at
        # 1 to avoid early-game scaling penalty on tight Ti maps.
        ...
        # v13_swarm_attack: copying #1 team's strategy from replay
        # bb683e62-8a1d-49a0-85fb-81571b2cab44.
        # Their strategy:
        #   - 4 builders spawned r=0-3 (immediate economy scale)
        #   - 22 builders total over the match (vs our 2-3)
        #   - 218 sentinels forward-deployed near enemy core
        #   - 1379 builder attacks (vs our 30) = continuous raider pressure
        #   - 11.4x Ti mining lead by mining denial of opponent
        # Differentiator vs v8: aggressive raider waves, much higher cap.
        self.target_miner_count_early = 2      # pre-defense miners
        self.miner_wave_interval = 60          # rounds between post-defense miner waves
        self.target_repair_patrol_count = 2    # 2 repair patrols (kept from v5)
        self.target_raider_count_max = 8       # raised from 4 to 8 (#1 team had 22 total)
    ...
    def _init_game(self, c: Controller):
        ...
        # v14_quad_econ: scale initial target_economy_count with map area.
        # #1 team's replay was on a 50x41 map (area=2050) with 4 builders
        # spawned r=0-3. Bands:
        #   area < 600         -> 1 econ (small maps; refinery-dependent)
        #   area 600-1099      -> 2 econ
        #   area 1100-1599     -> 3 econ
        #   area 1600+         -> 4 econ (large/wide maps; mining-dominated)
        try:
            area = int(map_w) * int(map_h)
        except Exception:
            area = 400
        if area < 600:
            scaled_econ = 1
            ...
            scaled_econ = 4
        if scaled_econ > self.target_economy_count:
            self.target_economy_count = scaled_econ
            log_event(
                c.get_current_round(), c.get_id(), "core",
                f"({self.core_pos.x},{self.core_pos.y})",
                "v14_economy_scaled", area=area, target=scaled_econ,
            )
    ...
    def _run_core(self, c: Controller):
        ...
        # v4_micky: late-game second economy bot. Once refinery has
        # bootstrapped and Ti pool is comfortably above the 2000 gate,
        # spawn a second economy bot to expand the harvester network.
        # Cost: 1 builder spawn (+20% scale), worthwhile if pool is
        # already large.
        # v5_swarm: also push to 3rd economy bot at rnd>=900 with bigger Ti
        # floor (>2500) — pool is plentiful by then, refinery delivered
        # multiple ax stacks, more harvester spread = more Ti per round.
        try:
            ti_late, _ = c.get_global_resources()
            if rnd >= 500 and ti_late > 1500 and self.target_economy_count < 2:
                self.target_economy_count = 2
            if rnd >= 900 and ti_late > 2500 and self.target_economy_count < 3:
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            # Repair patrol: v7_smart_repair gates 2nd patrol behind Ti pool.
            # 1st at r>=80 with harvester>=3 (same as v5).
            # 2nd ONLY when r>=600 AND Ti pool > 1500 — protects refinery
            # bootstrap. v5's 2nd-repair (r>=400, no Ti gate) added +20%
            # scale during foundry phase; this gates that.
            repair_count = len(self.repair_patrol_bot_ids)
            ...
            # v13_swarm_attack: aggressive raider waves matching #1 team.
            # - Spawn round dropped 150 -> 120 (earlier first wave)
            # - Ti floor +80 -> +40 (more aggressive spawning)
            # - Force-spawn schedule extended earlier: 200, 280, 360, 440,
            #   then every 100 to 1900. Drops floor to bare cost.
            # - Cap raised to 8 in late game (was 4).
            if (
                self.defense_complete
                and rnd >= 120
                and combat_econ_ready
                ...
                ti_floor = builder_cost if force_raider_round else (
                    builder_cost + 40
                )
