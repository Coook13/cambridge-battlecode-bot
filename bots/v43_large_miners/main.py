...
class Player:
    ...
    def _run_core(self, c: Controller):
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            miner_interval = 35 if map_area >= 1200 else self.miner_wave_interval
            miner_ti_floor = builder_cost + (5 if map_area >= 1200 else 50)
            miner_cap_ok = map_area < 1200 or miner_count < 10
            if (
                self.defense_complete
                and rnd >= 70
                and combat_econ_ready
                and miner_cap_ok
                and (rnd - self._last_miner_wave_round) >= miner_interval
                and ti >= miner_ti_floor
            ):
