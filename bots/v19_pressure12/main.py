...
def _max_raiders_for_round(rnd: int) -> int:
    ...
    v19_pressure12: push harder toward the #1 replay's constant attack
    count. This intentionally tests whether v17's economy ladder can pay
    for a 12-raider ceiling.
    """
    if rnd < 200:
        return 3
    if rnd < 400:
...
class Player:
    def __init__(self):
        ...
        self.target_raider_count_max = 12      # hard cap; actual cap by round bracket
    ...
    def _run_core(self, c: Controller):
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            # v19_pressure12: heavier pressure test. More frequent force
            # rounds and lower non-force floor, but still no spawning below
            # current builder cost.
            if (
                self.defense_complete
                and rnd >= 100
                and combat_econ_ready
                ...
                force_raider_round = rnd in (
                    ...
                    1320, 1400, 1480, 1560, 1640, 1720, 1800, 1880,
                )
                ti_floor = builder_cost if force_raider_round else builder_cost + 20
