#version2_symmetry_bridging_detour
...
SIEGE_MIN_AREA = 1400
SIEGE_TI_GATE = 900
SIEGE_MAX_BUILDERS = 4
SIEGE_ORBIT_DIST2 = 18
...
class Player:
    ...
    def _core(self, c):
        ...
        if round_num >= RUSH_START:
            ...
            if self._should_spawn_siege(c, round_num, ti):
                ...
                for d in ALL_DIRS:
                    ...
                    try:
                        if c.can_spawn(sp):
                            d2 = sp.distance_squared(enemy_t)
                            if d2 < best_d2:
                                best_d2 = d2
                                best_sp = sp
                    except Exception:
                ...
                if best_sp is not None:
                    ...
                    if c.can_place_marker(best_sp):
                        c.place_marker(best_sp, MARKER_SIEGE_FLAG)
                    self.siege_spawned += 1
                    self.num_spawned += 1
                    return
    ...
    def _should_spawn_siege(self, c, round_num, ti):
        if round_num < SIEGE_START:
            return False
        if self.siege_spawned >= SIEGE_MAX_BUILDERS:
            return False
        if c.get_map_width() * c.get_map_height() < SIEGE_MIN_AREA:
            return False
        return ti >= SIEGE_TI_GATE
    ...
    def _do_turret_assault(self, c, pos, target):
        ...
        if self.is_siege and self._try_forward_core_turret(c, pos, core_target, prefer_sentinel=False):
            return
    ...
    def _direction_toward(self, start, target):
        try:
            d = start.direction_to(target)
    ...
    def _try_forward_core_turret(self, c, pos, core_pos, prefer_sentinel=False):
        if core_pos is None or pos.distance_squared(core_pos) > SIEGE_ORBIT_DIST2:
            return False
        if c.get_action_cooldown() > 0:
            return False
        ...
        best_dir = None
        best_score = 10**18
        order = (EntityType.SENTINEL, EntityType.GUNNER) if prefer_sentinel else (EntityType.GUNNER, EntityType.SENTINEL)
        ...
        for tp in tiles:
            ...
            try:
                ...
                continue
            face_dir = self._direction_toward(tp, core_pos)
            for turret_type in order:
                try:
                    can_build = (
                        c.can_build_sentinel(tp, face_dir)
                        if turret_type == EntityType.SENTINEL
                        else c.can_build_gunner(tp, face_dir)
                    )
                except Exception:
                    can_build = False
                if not can_build:
        ...
        if best is None:
            return False
        try:
            if best_type == EntityType.SENTINEL:
                c.build_sentinel(best, best_dir)
            else:
                c.build_gunner(best, best_dir)
            self.plant_gun_pos = best
            self.plant_gun_facing = best_dir
            self.plant_done_round = c.get_current_round()
            self.assault_phase = 'done'
            return True
        except Exception:
