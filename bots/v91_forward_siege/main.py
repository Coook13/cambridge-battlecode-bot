...
from cambc import EntityType, GameError
...
class Player:
    ...
    def _choose_delegate(self, c):
        ...
        if key == (50, 41, 3, 39):
            self._delegate_name = "siege_no1_replay_seat_a"
            self._delegate = siege_impl.Player()
            return
        ...
        if core_xy not in seat_a_cores:
            if width >= 45 and height >= 35:
                self._delegate_name = "siege_large_unknown_seat_b"
                self._delegate = siege_impl.Player()
                return
            self._delegate_name = "xuan53_known_scale_seat_b"
            self._delegate = xuan53_impl.Player()
            return

        if width * height >= 1400:
            self._delegate_name = "siege_large_unknown_seat_a"
            self._delegate = siege_impl.Player()
            return

        self._delegate_name = "xuan79_unknown_seat_a"
        self._delegate = xuan79_impl.Player()
