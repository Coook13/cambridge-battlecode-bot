...
class Player:
    ...
    def _core(self, c):
        ...
        preferred = CARDINALS[slot_in_wave % 4]
        dirs = [preferred] + [d for d in ALL_DIRS if d != preferred]
    ...
    def _do_scout(self, c, pos):
        ...
        if moved is not None:
            ...
            return
        self.stuck_turns += 1
    # -- v70_home_ring: home defense helpers ----------------------------
    def _count_friendly_home_turrets(self, c):
        """Count friendly turrets (gunner/sentinel/breach) within d2<=18 of core."""
        if self.core_pos is None:
            return 0
        my_team = c.get_team()
        count = 0
        try:
            for bid in c.get_nearby_buildings():
                try:
                    ...
                    if et not in (EntityType.GUNNER, EntityType.SENTINEL, EntityType.BREACH):
                        continue
                    bp = c.get_position(bid)
                    if bp.distance_squared(self.core_pos) <= 18:
                        count += 1
                except Exception:
                    continue
        except Exception:
    ...
    def _try_build_home_sentinel(self, c, pos):
        """Try to build a sentinel adjacent to current pos facing outward.
        Outward = direction away from core."""
        if self.core_pos is None:
            return False
        # Determine outward direction (away from core)
        cx, cy = self.core_pos.x, self.core_pos.y
        dx = pos.x - cx
        dy = pos.y - cy
        # Pick best cardinal facing
        if abs(dx) >= abs(dy):
            fwd_dir = Direction.EAST if dx >= 0 else Direction.WEST
        else:
            fwd_dir = Direction.SOUTH if dy >= 0 else Direction.NORTH
        # Try build at adjacent tiles facing outward
        for d in CARDINALS:
            check = pos.add(d)
            try:
                if c.can_build_sentinel(check, fwd_dir):
                    c.build_sentinel(check, fwd_dir)
                    return True
            except Exception:
                pass
            try:
                if c.can_build_gunner(check, fwd_dir):
                    c.build_gunner(check, fwd_dir)
                    return True
            except Exception:
                pass
        return False
    ...
    def _do_outward(self, c, pos):
        ...
        # v72_smart_ring: home gunner ring with Ti gate.
        # Only build when (Ti >= 100) AND (we already have 1+ harvester)
        # AND home_turrets < 2 (just 2 gunners not 4).
        # v79_large_only: aggressive ring ONLY on large maps (>1000 area).
        # Default to v72's config for everything else.
        try:
            ...
            map_area = 100
        if map_area >= 1000:
            target_count, build_range, ti_gate = 4, 8, 80
        else:
            target_count, build_range, ti_gate = 2, 4, 100

        if c.get_action_cooldown() == 0 and self.core_pos is not None:
            d2_core = pos.distance_squared(self.core_pos)
            if d2_core <= build_range and self.harvest_count >= 1:
                try:
                    ti, _ = c.get_global_resources()
                except Exception:
                    ti = 0
                if ti >= ti_gate:
                    home_turrets = self._count_friendly_home_turrets(c)
                    if home_turrets < target_count:
                        if self._try_build_home_sentinel(c, pos):
                            return
