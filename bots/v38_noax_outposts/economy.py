...
def run_economy(c: Controller, state: EconomyState, local_map):
    ...
    if is_post_launch:
        if _maybe_build_large_map_outpost_sentinel(c, state, local_map, cur_xy, rnd, uid):
            return
...
def _select_post_launch_objective(_c: Controller, state: EconomyState, local_map) -> str:
    ...
    # to 6 pair-wins.
    map_area = int(getattr(local_map, "width", 0) or 0) * int(getattr(local_map, "height", 0) or 0)
    axionite_enabled = map_area < 1200
    if axionite_enabled and titanium_pool > 1500:
        known_ax = tuple(
...
def _pick_prelaunch_escape_goal(local_map, core_xy, cur_xy):
    ...
    width = int(getattr(local_map, "width", 0) or 0)
    height = int(getattr(local_map, "height", 0) or 0)
    if width * height <= 400 and width > 0 and height > 0:
        cx, cy = int(core_xy[0]), int(core_xy[1])
        dir_x = 1 if cx < ((width - 1) / 2.0) else -1
        dir_y = -1 if cy < ((height - 1) / 2.0) else 1
        preferred = (
            max(0, min(width - 1, cx + dir_x * _NO_LAUNCHER_ESCAPE_MIN_CHEB)),
            max(0, min(height - 1, cy + dir_y * _NO_LAUNCHER_ESCAPE_MIN_CHEB)),
        )
        if (
            _outside_future_barrier_ring(core_xy, preferred, min_cheb=_NO_LAUNCHER_ESCAPE_MIN_CHEB)
            and _is_general_movement_passable(local_map, preferred[0], preferred[1], preferred)
        ):
            return preferred
...
def _maybe_build_large_map_outpost_sentinel(
    c: Controller,
    state: EconomyState,
    local_map,
    cur_xy,
    ...
    if state.large_outpost_built:
        return False
    if rnd < 280 or c.get_action_cooldown() > 0:
        return False

    width = int(getattr(local_map, "width", 0) or 0)
    height = int(getattr(local_map, "height", 0) or 0)
    if width * height < 1200:
        state.large_outpost_built = True
        return False

    if _chebyshev(state.core_xy, cur_xy) < 7:
        return False

    if rnd - state.large_outpost_last_attempt_round < 18:
        return False
    ...
    if _nearby_friendly_turret(c, cur_xy, radius_sq=36):
        state.large_outpost_built = True
        return False

    state.large_outpost_last_attempt_round = rnd
    enemy_xy = _predicted_enemy_core_xy_for_map(state.core_xy, width, height)
    ...
    for dx, dy in _ADJACENT_DELTAS_8:
        ...
        direction = target.direction_to(Position(enemy_xy[0], enemy_xy[1]))
        if direction == Direction.CENTRE:
            continue
        candidates.append((_manhattan((tx, ty), enemy_xy), tx, ty, direction))

    candidates.sort()
    for _d, tx, ty, direction in candidates:
        target = Position(tx, ty)
        try:
            if c.can_build_sentinel(target, direction):
                c.build_sentinel(target, direction)
                state.large_outpost_built = True
                log_event(
                    rnd,
...
def _nearby_friendly_turret(c: Controller, cur_xy, radius_sq: int) -> bool:
    my_team = c.get_team()
    cx, cy = int(cur_xy[0]), int(cur_xy[1])
    try:
    ...
    for bid in buildings:
        try:
            if c.get_team(bid) != my_team:
                continue
            if c.get_entity_type(bid) not in (EntityType.GUNNER, EntityType.SENTINEL, EntityType.BREACH):
                continue
            p = c.get_position(bid)
            dx = p.x - cx
            dy = p.y - cy
...
def _predicted_enemy_core_xy_for_map(core_xy, width: int, height: int):
    cx, cy = int(core_xy[0]), int(core_xy[1])
    return (max(0, width - 1 - cx), max(0, height - 1 - cy))
