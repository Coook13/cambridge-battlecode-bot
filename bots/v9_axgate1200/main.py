...
class Player:
    ...
    def _run_core(self, c: Controller):
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            # Repair patrol: v5_swarm spawns up to 2 (was 1). 1st at r>=80,
            # 2nd at r>=400 once conveyor network has grown. More repair
            # coverage = enemy raid damage gets healed faster.
            repair_count = len(self.repair_patrol_bot_ids)
            if (
                self.defense_complete
                and rnd >= 80
                ...
                and (repair_count == 0 or rnd >= 400)
                and ti >= builder_cost + 30
            ):
                if self._spawn_builder_for_role(c, ROLE_REPAIR_PATROL):
                    return

            # Raiders: r >= 150, economy ready, count capped, Ti floor raised
            # to 80 so a raider spawn never kills a pending harvester build.
            # (v5_swarm: keeping default Ti floor +80, no force-spawn — raider
            # tuning is the other variant's experiment.)
            if (
                self.defense_complete
                and rnd >= 150
                and combat_econ_ready
                ...
                if raider_count < max_raiders and ti >= builder_cost + 80:
