...
from constants import (BUILDING_ENTITY_TYPES, DIRECTIONAL_ENTITY_TYPES,
                       ...
                       PASSABLE_TILES,
                       TURRET_ENTITY_TYPES, WALKABLE_BUILDING_ENTITY_TYPES)
...
class UnitLocalMap:
    ...
    def _compute_tile_passability(self, x: int, y: int) -> bool:
        return self.get(x, y) in PASSABLE_TILES
    ...
    def get_adjacency_neighbors(self, x: int, y: int):
        ...
        if not self.in_bounds(x, y):
            return ()
        if self.get(x, y) not in PASSABLE_TILES:
            return ()
        ...
        for dx, dy in self._ADJ_DELTAS_8:
            nx = x + dx
            ny = y + dy
            if self.in_bounds(nx, ny) and self.get(nx, ny) in PASSABLE_TILES:
                neighbors.append((nx, ny))
    ...
    def _entity_tile_code(self, entity_type: EntityType, team) -> int:
        ...
        if team != self.my_team:
            ...
            if entity_type in WALKABLE_BUILDING_ENTITY_TYPES:
                return MAP_TILE_BY_ENTITY.get(entity_type, MAP_ENEMY)
            return MAP_ENEMY
            ...
            for dx in range(-1, 2):
                ...
                if self.friendly_core_pos is not None:
                    if (
                        ...
                        continue
                self._set_inferred_tile(x, y, MAP_ENEMY, allow_observed=True)
