...
class Player:
    def __init__(self):
        ...
        self.map_w = None
        self.map_h = None
        self._sabotage_pending_xy = None
    ...
    def _is_enemy_transport_tile(self, c: Controller, pos: Position) -> bool:
        try:
            ...
            if c.get_team(bid) == c.get_team():
                return False
            return c.get_entity_type(bid) in (
                EntityType.ROAD,
                EntityType.CONVEYOR,
                EntityType.SPLITTER,
                EntityType.BRIDGE,
    ...
    def _try_builder_transport_sabotage(self, c: Controller) -> bool:
        """Cheap enemy-side pressure: overwrite enemy transport underfoot.

        v26: the #1 replay appears to win by keeping builders in the opponent's
        logistics. Turrets proved too expensive in our tests, so this hook lets
        any forward builder perform the cheap version: destroy enemy walkable
        transport and stamp a conveyor toward our core.
        """
        if self.role in (ROLE_DEFENDER, ROLE_HEALER):
            return False
        if self.core_pos is None or c.get_action_cooldown() != 0:
            return False
        ...
        if pos.distance_squared(self.core_pos) < 36:
            return False
        ...
        try:
            ...
            ti = 0
        if ti < 80 and rnd < 500:
            return False

        cur_xy = (pos.x, pos.y)
        out_dir = pos.direction_to(self.core_pos)

        if self._sabotage_pending_xy == cur_xy:
            try:
                if c.can_build_conveyor(pos, out_dir):
                    c.build_conveyor(pos, out_dir)
                    log_event(
                        rnd,
                        c.get_id(),
                        "builder",
                        f"({pos.x},{pos.y})",
                        "generic_sabotage_conveyor_built",
                    )
                    self._sabotage_pending_xy = None
                    return True
            except GameError:
        ...
        if not self._is_enemy_transport_tile(c, pos):
            return False

        try:
            if c.can_destroy(pos):
                c.destroy(pos)
                self._sabotage_pending_xy = cur_xy
                log_event(
                    rnd,
                    c.get_id(),
                    "builder",
                    f"({pos.x},{pos.y})",
                    "generic_sabotage_destroyed_transport",
                )
                return True
        except GameError:
            pass
        ...
        try:
            if c.can_build_conveyor(pos, out_dir):
                ...
                log_event(
                    ...
                    "builder",
                    f"({pos.x},{pos.y})",
                    "generic_sabotage_conveyor_built_direct",
                )
                return True
        except GameError:
            pass
    ...
    def _run_bot(self, c: Controller):
        ...
        if self._try_builder_transport_sabotage(c):
            return
