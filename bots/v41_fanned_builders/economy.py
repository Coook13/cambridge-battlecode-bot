...
def _run_post_launch_exploration(
    ...
    if state.phase == "explore_generate_waypoints":
        ...
        waypoints = _rotate_waypoints_for_builder(waypoints, uid)
...
def _rotate_waypoints_for_builder(waypoints, uid: int):
    if not waypoints:
        return waypoints
    n = len(waypoints)
    if n <= 1:
    ...
    if offset == 0:
        return waypoints
    return tuple(waypoints[offset:]) + tuple(waypoints[:offset])
