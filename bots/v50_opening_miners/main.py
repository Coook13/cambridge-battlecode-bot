...
class Player:
    ...
    def _late_builder_fire_priority(self, etype: EntityType) -> int:
    ...
    def _try_late_large_builder_fire(self, c: Controller) -> bool:
        ...
        try:
            ...
            return False
        if max(abs(pos.x - self.core_pos.x), abs(pos.y - self.core_pos.y)) < 10:
            return False
        ...
        for entity_id in nearby:
            try:
                ...
                prio = self._late_builder_fire_priority(etype)
                if prio >= 9:
                    continue
    ...
    def _spawn_round_cooldown_ready(self, rnd: int) -> bool:
        if not isinstance(self.last_role_spawn_round, int):
            return True
        min_gap = self.min_rounds_between_role_spawns
        if (
            isinstance(self.map_w, int)
            and isinstance(self.map_h, int)
            and self.map_w * self.map_h >= 1200
            and int(rnd) < 80
        ):
            min_gap = 1
        return (int(rnd) - int(self.last_role_spawn_round)) >= int(min_gap)
    ...
    def _run_core(self, c: Controller):
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            miner_count = len(self.miner_bot_ids)
            raider_count = len(self.raider_bot_ids)
            map_area = (self.map_w or 0) * (self.map_h or 0)
    ...
    def _run_bot(self, c: Controller):
        ...
        if self._try_late_large_builder_fire(c):
            return
