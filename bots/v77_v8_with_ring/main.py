...
def _max_raiders_for_round(rnd: int) -> int:
    ...
    Matches combat scheduler pacing: 1 before r200, 2 by mid-game,
    3 late, 4 after r800. Keeps Ti budget from being blown on
    raider spam when the economy is still bootstrapping.
    """
    if rnd < 200:
        return 1
    if rnd < 400:
...
class Player:
    def __init__(self):
        ...
        self.target_raider_count_max = 4       # hard cap; actual cap by round bracket
    ...
    def _run_core(self, c: Controller):
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            # Raiders: r >= 150, economy ready, count capped.
            # v8_combined: smart repair (from v7) + late force-spawn raider
            # (from v6) at r >= 700. Earlier force-spawns risked starving
            # foundry mid-build (~rnd 200-500). Late force-spawns add chaos
            # without breaking refined-ax pipeline.
            if (
                self.defense_complete
                and rnd >= 150
                and combat_econ_ready
