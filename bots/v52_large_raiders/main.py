    ...
    def _run_core(self, c: Controller):
        ...
        if c.get_action_cooldown() == 0 and self._spawn_round_cooldown_ready(rnd):
            ...
            if (
                ...
                if map_area >= 1200:
                    max_raiders = 12 if rnd >= 500 else _max_raiders_for_round(rnd)
                    force_raider_round = rnd in (
                        200, 280, 360, 440, 520, 600, 680, 760, 840,
                        920, 1000, 1080, 1160, 1240, 1320, 1400,
                        1480, 1560, 1640, 1720, 1800, 1880, 1960,
                    )
                    ti_floor = builder_cost + (20 if force_raider_round else 80)
                else:
