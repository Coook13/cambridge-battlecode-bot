...
class Player:
    ...
    def _builder_pressure_priority(self, etype: EntityType) -> int:
    ...
    def _try_builder_forward_attack(self, c: Controller) -> bool:
        """Let any forward builder chip enemy units/logistics.

        v27: the #1 replay and v13 both win by sustained builder attacks.
        Raiders already attack opportunistically, but economy/miner/repair
        builders also reach the fight. This hook gives those bodies teeth
        without adding unit-scale buildings or starving the foundry.
        """
        ...
        my_team = c.get_team()
        try:
            entity_ids = c.get_nearby_entities()
        except GameError:
            return False
        ...
        for entity_id in entity_ids:
            try:
                ...
                prio = self._builder_pressure_priority(etype)
                if prio >= 9:
                    continue
                target = c.get_position(entity_id)
                if not c.can_fire(target):
                    continue
            except GameError:
                continue
            d2 = pos.distance_squared(target)
            key = (prio, d2, target.x, target.y)
    ...
    def _run_bot(self, c: Controller):
        ...
        if self._try_builder_forward_attack(c):
            return
